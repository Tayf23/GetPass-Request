...
exports.handler = async function(event, context) {
  ...
  try {
    ...
    // Determine if we should expect a file download
    const isDownloadEndpoint = path.includes('/download-file/') || 
                              (path.includes('/generate-getpass/') && method === 'post');
    ...
    const response = await axios({
      ...
      validateStatus: () => true, // Accept all status codes
      timeout: 60000, // Increased timeout to 60 seconds
      // Set responseType based on endpoint
      responseType: isDownloadEndpoint ? 'arraybuffer' : 'json'
    });
    
    console.log('Response status:', response.status);
    console.log('Response headers:', response.headers);
    
    // Check if the response is binary by content type
    const isBinaryResponse = response.headers['content-type'] && (
      response.headers['content-type'].includes('application/vnd.openxmlformats') ||
      response.headers['content-type'].includes('application/octet-stream') ||
      response.headers['content-type'].includes('application/pdf')
    );
    ...
    // Check if we received a JSON response in arraybuffer format
    if (response.headers['content-type'] && 
      ...
      try {
        ...
        console.log('Converted arraybuffer to JSON successfully');
        ...
        console.log('Error parsing JSON from buffer:', e);
      }
    }
